...
def match_products(crop_path_or_url, top_k=1, is_url=False):
    """
    Matches a cropped image (from path or URL) to the catalog using CLIP+FAISS.
    Args:
        crop_path_or_url (str): Local path or URL to the cropped image.
        top_k (int): Number of top matches to return.
        is_url (bool): If True, treat crop_path_or_url as a URL and download in-memory.
    Returns:
        List of match dicts.
    """
    ...
    if is_url:
        image = download_image_to_pil(crop_path_or_url)
    else:
        image = Image.open(crop_path_or_url).convert("RGB")
