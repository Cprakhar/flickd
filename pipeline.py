            ...
            for file in files:
                if file.endswith("_detections.json"):
                    ...
                    for idx, det in enumerate(detections):
                        bbox = det["bbox"]
                        crop_name = f"{video_name}_{frame_name.replace('.jpg','')}_det{idx}.jpg"
                        crop_path = crop_and_save(frame_path, bbox, self.crops_dir, crop_name=crop_name)
                        matches = match_product(crop_path, top_k=top_k, is_url=False)
                        # Add to reel_matches
